...
func _create_tree_items_from_dictionary(
	tree: Tree, parent_item: TreeItem, data: Dictionary, data_column: int = 1
) -> void:
	for key in data:
		var value = data[key]
			...
			match mismatch.type:
				DebugStateComparer.MismatchType.MISSING:
					...
					if mismatch.local_state is Dictionary:
						...
						_create_tree_items_from_dictionary(
							tree, mismatch_item, _convert_array_to_dictionary(mismatch.local_state)
						)
					else:
				...
				DebugStateComparer.MismatchType.EXTRA:
					...
					if mismatch.remote_state is Dictionary:
						...
						_create_tree_items_from_dictionary(
							tree, mismatch_item, _convert_array_to_dictionary(mismatch.remote_state), 2
						)
					else:
						var child = tree.create_item(mismatch_item)
