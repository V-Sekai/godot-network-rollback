...
func _draw_peer(peer_id: int, peer_rect: Rect2, draw_data: Dictionary) -> void:
	...
	while frame.start_time <= absolute_end_time:
		var frame_rect = Rect2(
			Vector2(
				extended_peer_rect.position.x + frame.start_time - absolute_start_time, extended_peer_rect.position.y
			),
		...
		if frame_rect.intersects(extended_peer_rect):
				...
				if frame.data.has("skip_reason"):
					...
					match int(frame.data["skip_reason"]):
						...
						Logger.SkipReason.ADVANTAGE_ADJUSTMENT:
							tick_letter = "A"
					if tick_letter != "":
						tick_numbers_to_draw.append(
							[_font_big, center_position - Vector2(5, 0), tick_letter, Color("f04dff")]
						)
			else:
			...
			if show_rollback_ticks and frame.data.has("rollback_ticks"):
				var rollback_height = (
					extended_peer_rect.size.y * (float(frame.data["rollback_ticks"]) / float(max_rollback_ticks))
				)
				if rollback_height > extended_peer_rect.size.y:
					rollback_height = extended_peer_rect.size.y
				var rollback_point = Vector2(
					center_position.x, frame_rect.position.y + frame_rect.size.y - rollback_height
				)
...
func _draw() -> void:
	...
	for peer_index in range(peer_count):
		...
		var peer_rect := Rect2(Vector2(0, current_y), Vector2(size.x, peer_height))
	...
	if cursor_time >= start_time and cursor_time <= start_time + size.x:
		draw_line(
			Vector2(cursor_time - start_time, 0), Vector2(cursor_time - start_time, size.y), Color(1.0, 0.0, 0.0), 3.0
		)
