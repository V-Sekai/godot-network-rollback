			...
			if data["log_type"] == Logger.LogType.HEADER:
				...
				if header["peer_id"] in peer_ids:
					...
					call_deferred(
						"emit_signal",
						"load_error",
						"Log file has data for peer_id %s, which is already loaded" % header["peer_id"]
					)
					_set_loading(false)
					return
				...
				if match_info.size() > 0 and match_info.hash() != header_match_info.hash():
					...
					call_deferred(
						...
						"load_error",
						(
							"Log file for peer_id %s has match info that doesn't match already loaded data"
							% header["peer_id"]
						)
					)
					_set_loading(false)
	...
	match log_entry["log_type"] as int:
		...
		Logger.LogType.FRAME:
			...
			if log_entry.has("start_time"):
				...
				peer_start_times[peer_id] = (
					int(min(peer_start_time, frame_data.start_time)) if peer_start_time > 0 else frame_data.start_time
				)
