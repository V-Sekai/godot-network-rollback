...
class InputForPlayer:
	...
	func _init(_input: Dictionary, _predicted: bool):
		input = _input
...
class StateHashFrame:
	...
	func _init(_tick: int, _state_hash: int):
		tick = _tick
...
# In seconds, because we don't want it to be dependent checked the network tick.
...
func _ready() -> void:
	#get_tree().connect("peer_disconnected",Callable(self,"remove_peer"))
	#get_tree().connect("server_disconnected",Callable(self,"stop"))
	...
	_ping_timer.timeout.connect(Callable(self, "_on_ping_timer_timeout"))
	add_child(_ping_timer)

	_spawn_manager = SpawnManager.new()
	_spawn_manager.name = "SpawnManager"
	add_child(_spawn_manager)
	_spawn_manager.scene_spawned.connect(Callable(self, "_on_SpawnManager_scene_spawned"))
	_spawn_manager.scene_despawned.connect(Callable(self, "_on_SpawnManager_scene_despawned"))
	...
	if message_serializer == null:
		set_message_serializer(
			_create_class_from_project_settings(
				"network/rollback/classes/message_serializer", DEFAULT_MESSAGE_SERIALIZER_PATH
			)
		)
	if hash_serializer == null:
		set_hash_serializer(
			_create_class_from_project_settings(
				"network/rollback/classes/hash_serializer", DEFAULT_HASH_SERIALIZER_PATH
			)
		)
...
func reset_network_adaptor() -> void:
	set_network_adaptor(
		_create_class_from_project_settings("network/rollback/classes/network_adaptor", DEFAULT_NETWORK_ADAPTOR_PATH)
	)
...
func start() -> void:
	assert(network_adaptor.is_network_host() or mechanized)  #,"start() should only be called checked the host")
	...
	if network_adaptor.is_network_host():
		...
		# Call _remote_start() checked all the other peers.
		for peer_id in peers:
			network_adaptor.send_remote_start(peer_id)

		# Attempt to prevent double starting checked the host.
		_host_starting = true
...
func _call_get_local_input() -> Dictionary:
	...
	for node in nodes:
		if (
			network_adaptor.is_network_master_for_node(node)
			and Utils.has_interop_method(node, "_get_local_input")
...
func _call_save_state() -> Dictionary:
	...
	for node in nodes:
		if (
			Utils.has_interop_method(node, "_save_state")
			and node.is_inside_tree()
			and not node.is_queued_for_deletion()
		):
...
func _predict_missing_input(input_frame: InputBufferFrame, previous_frame: InputBufferFrame) -> InputBufferFrame:
	if not input_frame.is_complete(peers):
		...
		for peer_id in missing_peers:
			...
			missing_peers_ticks_since_real_input[peer_id] = (
				-1 if peer.last_remote_input_tick_received == 0 else current_tick - peer.last_remote_input_tick_received
			)
		var nodes: Array = get_tree().get_nodes_in_group("network_sync")
		for node in nodes:
			...
			if has_predict_network_input or previous_input.has(node_path_str):
				...
				var predicted_input_for_node = (
					Utils.try_call_interop_method(
						node, "_predict_remote_input", [previous_input_for_node, ticks_since_real_input]
					)
					if has_predict_network_input
					else previous_input_for_node.duplicate()
				)
...
func _cleanup_buffers() -> bool:
	...
	while state_buffer.size() > max_buffer_size + 1:
		...
		if input_frame == null:
			var message = (
				"Attempting to retire state frame %s, but input frame %s is missing"
				% [state_frame_to_retire.tick, state_frame_to_retire.tick + 1]
			)
		...
		if not input_frame.is_complete(peers):
			...
			var message = (
				"Attempting to retire state frame %s, but input frame %s is still missing input (missing peer(s): %s)"
				% [state_frame_to_retire.tick, input_frame.tick, missing]
			)
			push_warning(message)
			if _logger:
				_logger.data["buffer_underrun_message"] = message
		...
		if state_frame_to_retire.tick > _last_state_hashed_tick:
			var message = (
				"Unable to retire state frame %s, because we haven't hashed it yet" % state_frame_to_retire.tick
			)
			push_warning(message)
			if _logger:
				_logger.data["buffer_underrun_message"] = message
	...
	while state_hashes.size() > (max_buffer_size * 2):
		...
		if not state_hash_to_retire.is_complete(peers) and not mechanized:
			...
			var message = (
				"Attempting to retire state hash frame %s, but we're still missing hashes (missing peer(s): %s)"
				% [state_hash_to_retire.tick, missing]
			)
			push_warning(message)
			if _logger:
				_logger.data["buffer_underrun_message"] = message
...
func _get_input_messages_from_send_queue_for_peer(peer: Peer) -> Array:
	...
	return (
		_get_input_messages_from_send_queue_in_range(
			last_index - (new_messages * max_input_frames_per_message) + 1, last_index, true
		)
		+ _get_input_messages_from_send_queue_in_range(
			first_index, first_index + (old_messages * max_input_frames_per_message) - 1
		)
	)
...
func _physics_process(_delta: float) -> void:
	...
	if mechanized:
		...
		# to the previous tick in order to sample.
	...
	if rollback_ticks > 0:
		...
		if debug_check_local_state_consistency:
			...
			_debug_check_local_state_consistency_buffer = state_buffer.slice(
				state_buffer.size() - rollback_ticks - 1, state_buffer.size() - 1
			)
			# Debug check that states computed multiple times with complete inputs are the same
		...
		# Iterate forward until we're at the same spot we left unchecked.
		...
		if peers.size() > 0:
			...
			if debug_check_message_serializer_roundtrip:
				...
				if local_input["$"] != unserialized_input["$"]:
					push_error(
						(
							"The input is different after being serialized and unserialized \n Original: %s \n Unserialized: %s"
							% [ordered_dict2str(local_input), ordered_dict2str(unserialized_input)]
						)
					)
	...
	if current_tick > 0:
		...
		if interpolation:
			# Capture the state data to sample between.
			...
			# Return to state from the previous frame, so we can sample
...
func _process(delta: float) -> void:
	...
	if not interpolation or not _ran_physics_process or _ticks_since_last_interpolation_frame > 1:
		...
		# Don't sample if we are skipping ticks, or just ran physics process.
...
func _on_received_input_tick(peer_id: int, serialized_msg: PackedByteArray) -> void:
	...
	peer.next_local_input_tick_requested = max(
		msg[MessageSerializer.InputMessageKey.NEXT_INPUT_TICK_REQUESTED], peer.next_local_input_tick_requested
	)
		...
		if state_hash_frame and not state_hash_frame.has_peer_hash(peer_id):
			if not state_hash_frame.record_peer_hash(peer_id, remote_state_hashes[remote_tick]):
				emit_signal(
					...
					remote_tick,
					peer_id,
					state_hash_frame.state_hash,
					remote_state_hashes[remote_tick]
				)
	...
	peer.next_local_hash_tick_requested = max(
		msg[MessageSerializer.InputMessageKey.NEXT_HASH_TICK_REQUESTED], peer.next_local_hash_tick_requested
	)
...
func spawn(
	name: String, parent: Node, scene: PackedScene, data: Dictionary = {}, rename: bool = true, signal_name: String = ""
) -> Node:
	if not started:
...
func _debug_check_consistent_local_state(state: StateBufferFrame, message := "Loaded") -> void:
	...
	if previously_hashed_frame and previously_hashed_frame.state_hash != hashed_state:
		...
		push_error(
			(
				"%s state is not consistent with saved state:\n %s"
				% [
					message,
