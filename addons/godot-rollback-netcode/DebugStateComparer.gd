...
class Mismatch:
	...
	func _init(_type: int, _path: String, _local_state, _remote_state):
		type = _type
		path = _path
...
func find_mismatches(local_state: Dictionary, remote_state: Dictionary) -> void:
	_find_mismatches_recursive(_clean_up_state(local_state), _clean_up_state(remote_state))
...
func _find_mismatches_recursive(local_state: Dictionary, remote_state: Dictionary, path: Array = []) -> void:
	...
	for key in local_state:
		if not remote_state.has(key):
			...
			mismatches.append(
				Mismatch.new(MismatchType.MISSING, _get_diff_path_string(path, key), local_state[key], null)
			)
	...
	for key in remote_state:
		if not local_state.has(key):
			missing_or_extra = true
			mismatches.append(
				Mismatch.new(MismatchType.EXTRA, _get_diff_path_string(path, key), null, remote_state[key])
			)
	...
	if not missing_or_extra:
		if local_state.keys() != remote_state.keys():
			mismatches.append(
				Mismatch.new(
					MismatchType.REORDER, _get_diff_path_string(path, "KEYS"), local_state.keys(), remote_state.keys()
				)
			)
			...
			if remote_value is Array:
				if local_value != remote_value:
					_find_mismatches_recursive(
						_convert_array_to_dictionary(local_value),
						_convert_array_to_dictionary(remote_value),
						_extend_diff_path(path, key)
					)
			else:
				_add_diff_mismatch(local_value, remote_value, path, key)
...
func _add_diff_mismatch(local_value, remote_value, path: Array, key) -> void:
	mismatches.append(
		Mismatch.new(MismatchType.DIFFERENCE, _get_diff_path_string(path, key), local_value, remote_value)
	)
