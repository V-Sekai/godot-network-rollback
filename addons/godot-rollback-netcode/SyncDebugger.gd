...
func _on_SyncManager_prediction_missed(
	tick: int, peer_id: int, local_input: Dictionary, remote_input: Dictionary
) -> void:
	print("-----")
...
func _on_SyncManager_remote_state_mismatch(tick: int, peer_id: int, local_hash: int, remote_hash: int) -> void:
	...
	print(
		(
			"On tick %s, remote state (%s) from %s doesn't match local state (%s)"
			% [tick, remote_hash, peer_id, local_hash]
		)
	)
...
func _on_SyncManager_peer_pinged_back(peer: SyncManager.Peer) -> void:
	...
	print(
		(
			"Peer %s: RTT %s ms | local lag %s | remote lag %s | advantage %s"
			% [peer.peer_id, peer.rtt, peer.local_lag, peer.remote_lag, peer.calculated_advantage]
		)
	)
	if _debug_overlay:
